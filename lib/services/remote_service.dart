...
class RemoteService {
  Future<List<Closest>?> getClosest() async {
    ...
    var uri = Uri.parse('https://api.hipspot.xyz/cafe/recommend/nearby?startLng=$long&startLat=$lat');
  ...
  Future<List<HighRated>?> getHighRated() async {
    ...
    var uri = Uri.parse('https://api.hipspot.xyz/cafe/recommend/high-rated');
    var response = await client.get(uri);
    if (response.statusCode == 200) {
      var json = response.body;
  ...
  Future<List<NewlyOpened>?> getNewlyOpened() async {
    ...
    var uri = Uri.parse('https://api.hipspot.xyz/cafe/recommend/new?startLng=$long&startLat=$lat');
